...
class AWSAuthConnection:
    ...
    def has_key(self, bucket, key):
        l = self.list_bucket(bucket, options={'prefix': key})
        #print 'i can haz %s: %s' % (key, ' '.join([x.key for x in l.entries]))
    ...
    def put_canned_acl(self, bucket, key, acl):
        return self.put_acl(bucket, key, '', headers={ 'x-amz-acl': acl })
